...
{       char *memerr = "Unable to allocate memory.";
        if((tokenbuf=malloc(sizeof(char)*32))==NULL)
            fatal(memerr);
        tokenptr=tokenbuf;
        tokenend=tokenbuf;
        Course *c;
...
Name readname()
{
        Name n;
            ...
            if(istoken()){
                n= newstring(tokenptr, tokensize());
            }
            else {
...
void advancetoken()
{
        ...
        while((c = getc(ifile->fd)) != EOF) {
                ...
                if(i==size){
                    size*=2;
                    tokenbuf = realloc(tokenbuf, size * sizeof(char));
                    tokenptr=tokenbuf;
                    tokenend=tokenbuf;
                    tokenend=tokenend+i;
                    *(tokenend)=c;
...
void advanceeol()
{
        ...
        if(i==size){
            tokenbuf = realloc(tokenbuf, (size+1) * sizeof(char));
            tokenptr=tokenbuf;
            tokenend=tokenbuf;
            tokenend=tokenend+i;
            *(tokenend)='\0';
